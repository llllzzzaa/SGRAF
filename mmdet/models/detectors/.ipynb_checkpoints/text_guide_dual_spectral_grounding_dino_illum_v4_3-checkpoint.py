...
class TextDualSpectralGroundingDINOillumV4_3(DINO):
    ...
    def __init__(self,
        ...
        self.fusion_module = RGBTFusionModule(
            in_channels=[192, 384, 768], text_dim=256 
        )
    ...
    def loss(self, batch_inputs: Tensor,
        ...
        scene_vals = [data_sample.metainfo['scene'] for data_sample in batch_data_samples]
        ...
        if 'tokens_positive' in batch_data_samples[0]:
            ...
            if len(set(text_prompts)) == 1:
                ...
                tokenized, caption_string, tokens_positive, _ = \
                    self.get_tokens_and_prompts(
                        text_prompts[0], True)
                new_text_prompts = [caption_string] * len(batch_inputs['img'])  #batch_inputs
        ...
        for i, data_samples in enumerate(batch_data_samples):
            positive_map = positive_maps[i].to(
                batch_inputs['img'].device).bool().float()  #batch_inputs
        ...
        if self.use_autocast:
            with autocast(enabled=True):
                visual_features, illum_score = self.extract_feat(batch_inputs,text_dict)  #batch_inputs
        else:
            visual_features, illum_score = self.extract_feat(batch_inputs,text_dict)  #batch_inputs
        ...
        # 1. 直接用列表推导式提取整个 batch 的 scene 值 (此时为 [0, 1, 0, 0] 这样的整数列表)
        # scene_vals = [data_sample.metainfo['scene'] for data_sample in batch_data_samples]

        # 2. 转换为 Tensor 并放到对应的 GPU 上
        device = illum_score.device
        scene_tensor = torch.tensor(scene_vals, dtype=torch.float32, device=device)

        # 3. 数值映射与维度对齐：
        # 利用公式 1.0 - scene_tensor 直接完成 0->1.0 和 1->0.0 的翻转
        # 然后 unsqueeze(1) 将维度从 [B] 扩充为 [B, 1]
        target_scene = (1.0 - scene_tensor).unsqueeze(1)

        # 4. 计算 BCE Loss
        # 注意：请确保传入的 illum_score 是已经映射到 [0, 1] 区间的概率值
        # 如果你拿的是之前 tanh 输出的 illum_dir，记得先做: illum_score = (illum_dir + 1.0) / 2.0
        loss_illum = F.binary_cross_entropy(illum_score, target_scene)
        losses['loss_illum'] = loss_illum * 0.1
        return losses
...
class TextGuidedCrossSpectralAttention(nn.Module):
    def __init__(self,
        ...
        self.gate_adapt_rgb = nn.Conv2d(embed_dims, num_heads, kernel_size=1)
        self.gate_adapt_ir  = nn.Conv2d(embed_dims, num_heads, kernel_size=1)
    ...
    def forward(self, x_rgb, x_ir, text_feat, illum_score, text_mask=None):
        ...
        gate_rgb = self.gate_adapt_rgb(w_rgb)
        gate_ir  = self.gate_adapt_ir(w_ir)

        # 为了保证注意力加权后的总概率分布稳定，进行简单的归一化 (类似 Softmax/Sigmoid 的作用)
        gates = torch.stack([gate_rgb, gate_ir], dim=-1) # [B, num_heads, 1, 1, 2]
        gates = gates.softmax(dim=-1)
        
        norm_gate_rgb = gates[..., 0] # [B, num_heads, 1, 1]
        norm_gate_ir  = gates[..., 1] # [B, num_heads, 1, 1]

        # RGB / IR complementary
        attn = norm_gate_rgb * attn_rgb + norm_gate_ir * attn_ir
        v = norm_gate_rgb * v_rgb + norm_gate_ir * v_ir

        text_visual_feat = torch.matmul(attn, v)

        out = torch.matmul(attn.transpose(-2, -1), text_visual_feat)

        out = out.permute(0, 1, 3, 2).reshape(B, C, H, W)  # [B,L,C]
...
class RGBTFusionModule(nn.Module):
    def __init__(self, in_channels=[192, 384, 768], text_dim=256, eval_size=(640, 640)):
        super().__init__()
        ...
        dat_kernels = [9, 7, 5]
        nat_kernels = [5, 5, 5]
        # window_sizes = [4, 4, 4] # 虽然定义了但在 AttentionBlock2 初始化没用到，先保留逻辑
        
        self.align_layers = nn.ModuleList()      # 负责几何对齐 (AttentionBlock2)
        self.fusion_layers = nn.ModuleList()     # 负责文本指导融合 (SpatialTextGuidedFusionLayer)
        
        for i, dim in enumerate(in_channels):

            if dim == 192:
                cur_heads = 4  # 针对 P3 层的特殊优化
            elif dim == 384:
                cur_heads = 8
            else:
                cur_heads = 8 # 或者 8，取决于显存，P5层其实8也可以

            # 1. 计算下采样后的尺寸 (用于 RPE)
            downsample = 2 ** (i + 3)
            current_size = (eval_size[0] // downsample, eval_size[1] // downsample)
            
            # 2. 构建对齐层 (AttentionBlock2 - 保持不变)
            ...
            # 3. 构建文本指导融合层 (替代原来的 conv2d)
            # 注意：Spatial Fusion 比较耗显存，如果在 P3 (stride=8) 显存不够，
            # 可以只在 P4, P5 使用 Spatial，P3 使用 Channel。
            # 这里默认全部使用 Spatial。
            # fusion_blk = SpatialTextGuidedFusionLayer(dim=dim, text_dim=text_dim)  
            fusion_blk = TextGuidedCrossSpectralAttention(
                embed_dims=dim,      # 对应循环当前的通道数
                text_dims=text_dim,  # 对应文本特征维度
                num_heads=8          # 192, 384, 768 都能被 8 整除
                #num_heads=cur_heads
            )
    ...
    def forward(self, x_rgb_list, x_ir_list, text_dict, illum_score):
        ...
        for i, (rgb, ir) in enumerate(zip(x_rgb_list, x_ir_list)):
            # 1. 几何对齐 (Physical Alignment) - Cross Deformable Attn
            rgb_f, ir_f = self.align_layers[i](rgb, ir)
            # 2. 文本指导的空间融合 (Semantic Spatial Fusion)
