...
class DETRHead(BaseModule):
    ...
    def loss_by_feat_single(self, cls_scores: Tensor, bbox_preds: Tensor,
        ...
        # ------------ BEGIN PATCH ------------
        # classification loss already prepared: cls_scores reshape and cls_avg_factor

        if isinstance(self.loss_cls, QualityFocalLoss):
            bg_class_ind = self.num_classes
            # positive indices (labels in [0, num_classes-1])
            pos_inds = ((labels >= 0) & (labels < bg_class_ind)).nonzero().squeeze(1)
            # prepare scores vector same shape as labels (1D)
            scores = label_weights.new_zeros(labels.shape)

            # only compute IoU scores when have positive indices
            if pos_inds.numel() > 0:
                # ensure bbox_preds is reshaped into (-1,4)
                bbox_preds_reshaped = bbox_preds.reshape(-1, 4)
                pos_bbox_targets = bbox_targets[pos_inds]
                pos_decode_bbox_targets = bbox_cxcywh_to_xyxy(pos_bbox_targets)
                pos_bbox_pred = bbox_preds_reshaped[pos_inds]
                pos_decode_bbox_pred = bbox_cxcywh_to_xyxy(pos_bbox_pred)
                # compute aligned IoU and fill into scores[pos_inds]
                iou = bbox_overlaps(
                    pos_decode_bbox_pred.detach(), pos_decode_bbox_targets, is_aligned=True)
                scores[pos_inds] = iou.to(scores.dtype)

            # call QFL with tuple (labels, scores)
            loss_cls = self.loss_cls(
                cls_scores, (labels, scores), weight=label_weights, avg_factor=cls_avg_factor)
        else:
            # fallback to original behavior (CE / Focal etc.)
