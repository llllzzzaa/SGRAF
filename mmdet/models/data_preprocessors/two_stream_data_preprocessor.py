...
class DualSteramDetDataPreprocessor(_DualSteramDetDataPreprocessor):  # img, img_ir
    ...
    def forward(self, data: dict, training: bool = False) -> dict:
        ...
        # if inputs_ir.shape[1] == 1:
        #     inputs_ir_vis = inputs_ir.repeat(1, 3, 1, 1)  # [B, 3, H, W] for 可视化
        # else:
        #     inputs_ir_vis = inputs_ir

        # # 合并为 4 通道输入
        # combined = torch.cat([inputs, inputs_ir], dim=1)  # [B, 4, H, W]

        # print(f'[Preprocessor] RGB shape: {inputs.shape}, IR shape: {inputs_ir.shape}, Combined shape: {combined.shape}')
        # print(f'[Preprocessor] Sample pixel value: {combined[0, :, :3, :3]}')
        # print(f'[Preprocessor] data_samples: {data_samples}')

        # # 保存为 numpy 方便外部可视化
        # os.makedirs('/root/autodl-tmp/UniRGB-IR/pre_debug', exist_ok=True)
        # np.save('/root/autodl-tmp/UniRGB-IR/pre_debug/sample_combined_input.npy', combined[0].cpu().numpy())

        # # 可视化保存
        # rgb_img = inputs[0].permute(1, 2, 0).cpu().numpy()
        # ir_img = inputs_ir_vis[0].permute(1, 2, 0).cpu().numpy()

        # plt.subplot(1, 2, 1)
        # plt.title('RGB')
        # plt.imshow(np.clip(rgb_img / 255, 0, 1))  # 若是归一化过的图像

        # plt.subplot(1, 2, 2)
        # plt.title('IR')
        # plt.imshow(np.clip(ir_img / 255, 0, 1), cmap='gray')

        # plt.savefig('/root/autodl-tmp/UniRGB-IR/pre_debug/rgb_ir_debug.png')
        # plt.close()
