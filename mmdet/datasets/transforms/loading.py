...
class LoadAlignedImagesFromFile(LoadImageFromFile):
    """
    Load aligned(registered) two images from file.
    ...
    def transform(self, results: dict) -> Optional[dict]:
        ...
        try:
            if self.file_client_args is not None:
                ...
                file_ir_client = fileio.FileClient.infer_client(
                    self.file_client_args, filename_ir)
                img_ir_bytes = file_client.get(filename_ir)
            else:
                img_bytes = fileio.get(
                    filename, backend_args=self.backend_args)
                img_ir_bytes = fileio.get(
                    filename_ir, backend_args=self.backend_args)
            img = mmcv.imfrombytes(
                img_bytes, flag=self.color_type, backend=self.imdecode_backend)
            img_ir = mmcv.imfrombytes(
                img_ir_bytes, flag=self.color_type, backend=self.imdecode_backend)
...
@TRANSFORMS.register_module()
class LoadAlignedImagesFromFile_IR_JFT(LoadImageFromFile):
    """
    Load aligned(registered) two images from file.
    ...
    def transform(self, results: dict) -> Optional[dict]:
        ...
        ir_type = img_ir.dtype
        img_ir = np.mean(img_ir, axis=2, keepdims=True)
        img_ir = cv2.applyColorMap(img_ir.astype(np.uint8), cv2.COLORMAP_JET)
        img_ir = img_ir.astype(ir_type)  # BGR, to be transformed to RGB in datapreprocess.
...
@TRANSFORMS.register_module()
class LoadFromAlignedImagesFromFile_IR_Only(LoadImageFromFile):
    """
    Load aligned(registered) two images from file.
...
class LoadAnnotations(MMCV_LoadAnnotations):
    ...
    def _format_polygon_mask(self, mask_ann):
        """Convert polygon masks to COCO-compatible Python list format.

        Args:
            mask_ann (list | dict): Polygon annotation.

        Returns:
            list | dict: Formatted polygon mask.
        """
        # polygon case
        if isinstance(mask_ann, list):
            formatted = []
            for poly in mask_ann:
                if isinstance(poly, np.ndarray):
                    formatted.append(poly.tolist())
                elif isinstance(poly, (list, tuple)):
                    formatted.append(list(poly))
                else:
                    raise TypeError(
                        f'Unsupported polygon type: {type(poly)}'
                    )
            return formatted

        # RLE case (leave untouched)
        return mask_ann

    def fix_polygon(self,mask_ann):
        fixed = []
        for poly in mask_ann:
            fixed.append(
    ...
    def _poly2mask(self, mask_ann: Union[list, dict], img_h: int,
        ...
        """
        #mask_ann = self._format_polygon_mask(mask_ann)
        #mask_ann = [[335.0, 213.0, 361.0, 213.0, 361.0, 287.0, 335.0, 287.0, 335.0, 213.0]]   
        #mask_ann = self.fix_polygon(mask_ann) 
        if isinstance(mask_ann, list):
